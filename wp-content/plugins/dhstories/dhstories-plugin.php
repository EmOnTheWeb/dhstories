<?php
/*
Plugin Name: Browse by Author widget 
Description: Add a browse by author widget to digihitch stories 
Author:Emilie 
*/
...
class WP_Widget_Author extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			...
			'description' => __( 'A dropdown of Authors' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct('author_dropdown', __('Author'), $widget_ops);
	}
	...
	public function widget( $args, $instance ) {
	 ...
	 // wp_list_authors('show_fullname=1&optioncount=1&orderby=post_count&order=DESC&style=none');
		...
		<select id="<?php echo esc_attr( $dropdown_id ); ?>" name="authors-dropdown" onchange='document.location.href=this.options[this.selectedIndex].value;'>
			<?php
			/**
			 ...
			 */
			// $dropdown_args = apply_filters( 'wp_dropdown_cats', array(
			// 	'style' => 'list',
			// 	'orderby'  => 'name', 
   //  			'order'    => 'ASC',
   //  			'show_post_count' => $c
				...
				foreach ($users as $user) 
				{
					if($user->display_name !== 'admin') {
						...
						$postCount = count_user_posts( $user->id);
					 	echo "<option value='".$url."'>".$authorName." (".$postCount.") </option>"; 	
					} 	
				}
			?>
